...
class ClientConnection:
    ...
    def __init__(
        self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, client_id: str
    ):
        self.reader = reader
        self.writer = writer
    ...
    async def send(self, message_id: str, args: Optional[dict] = None) -> bool:
        ...
        try:
            ...
            print(
                f"[CONNECTION RESET] Failed to send to client {self.client_id} - connection reset"
            )
            return False
        except BrokenPipeError:
            print(
                f"[BROKEN PIPE] Failed to send to client {self.client_id} - broken pipe"
            )
            return False
        except Exception as error:
            print(
                f"[SEND ERROR] Failed to send message to client {self.client_id}: {error}"
            )
            logging.error(f"Failed to send message to client {self.client_id}: {error}")
            return False
    ...
    def close(self):
        ...
        try:
            ...
            print(
                f"[CLOSE ERROR] Failed to close connection for client {self.client_id}"
            )
            pass
    ...
    async def wait_closed(self):
        ...
        try:
            ...
            print(
                f"[WAIT CLOSED ERROR] Failed to wait for close for client {self.client_id}"
            )
            pass
...
class BaseServer(ABC):
            ...
            try:
                ...
                try:
                    data = await asyncio.wait_for(
                        client.reader.read(32768), timeout=60.0
                    )
                ...
                if not self.quiet:
                    print(
                        f"[CONNECTION RESET] Client {client.client_id} connection reset by peer"
                    )
                break
            except Exception as error:
                if not self.quiet:
                    print(
                        f"[ERROR] Client {client.client_id} message loop error: {error}"
                    )
                self.logger.error(
                    f"Error in message loop for client {client.client_id}: {error}"
                )
                break
    ...
    async def _disconnect_client(self, client_id: str) -> None:
        ...
        if client_id in self.clients:
            ...
            self.logger.info(
                f"👋 Client disconnected: {client.address} (ID: {client_id})"
            )
    ...
    async def send_to_client(
        ...
        if client_id in self.clients:
            try:
                ...
                self.logger.error(
                    f"Error sending message to client {client_id}: {error}"
                )
                return False
        return False
    ...
    async def on_client_data(
        ...
        print(
            f"Received message {message_id} from client {client.client_id} with args {args}"
        )
        pass
